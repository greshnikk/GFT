...
public class SemanticAnalysis {
	...
	public static String convert(String input) throws Exception {
		...
		for (int i = 0; i < lexems.length; ++i) {
			if (lexems[i].startsWith("N") || lexems[i].startsWith("I")
					|| lexems[i].startsWith("C")) {
				output += lexems[i] + SPACE;
				continue;
			} else if (lexems[i].startsWith("F")) {
				output += lexems[i] + SPACE;
			} else if (lexems[i].equals("S1")) { // S1 = '(' TODO: Add method
													// stringToLexeme
				// for not using "S1"
				...
				st.push(lexems[i]);
			} else if (GFT.getPriority(st.lastElement()) < GFT
					.getPriority(lexems[i])) {
				st.push(lexems[i]);
			} else {
				while (!st.empty()
						&& GFT.getPriority(st.lastElement()) >= GFT
								.getPriority(lexems[i])) {
					output += st.pop() + SPACE;
				}
				st.push(lexems[i]);
